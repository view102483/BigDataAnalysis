...
names(NBA1415)
head(NBA1415)#想知道資料型態或資料長什麼樣子，可以幹麻？
#tail()看最後六個
nrow(NBA1415) #幾個球員
ncol(NBA1415)
#17
...
weekdays(x16)
months(x16)
seq(Sys.Date(), by="1 months", length.out=12)#每隔一個月輸出
seq(as.Date("2016-01-31"), by="1 months", length.out=12)#不夠會往後加一個月
#18
x17<-Sys.time()
x17
LisDate<-as.POSIXlt(x17)#存在list中
IntDate<-as.POSIXct(x17)#存下來是integer
LisDate
IntDate
unclass(LisDate)
unclass(IntDate)
